...
## Task number 1 - Renaming the column 
...
# Here i rename the column to "Homicide" (the reason i dont use the Dplyr method, is so its a permanent change - for future use)
...
## Task number 2 - New visualisation

```{r - new visualisation}
colnames(Western_Europe)[colnames(Western_Europe) == "Entity"] <- "Country"

# Firstly i change the column name from "Entity" to "Country" as this is a more accurate description of the values

lterm_trend <- ggplot(Western_Europe, aes(x=Year, y=Homicides, color = Country)) +
  geom_line() 

# Here i create a visualisation that better shows the long term trend in the different countries (x axis is values from "Year",y axis is values from "Homicides" and color is values from "Country").  

# I then call the visualisation, so it shows.
lterm_trend

# The reason i put the visualisation in a variable/vector, is so it is easier to work with in future tasks (no need to write the entire code everytime).
````

3) Can you change the facet layout to two columns and three rows so that the trends are more visible? 

## Task number 3 - Visualisation with facets

```{r - more facets}

lterm_trend_facet <- lterm_trend + facet_wrap(~Country, nrow = 3)

# Here i change the facet layout by grabbing the visualisation from earlier, and using "facet_wrap", to split it on the values from "Country" with 3 rows (nrow = 3).  

# Then call the visualisation so it shows.
lterm_trend_facet
```
...
## Task number 4 - Editing the legend

```{r - editing the legend}

# The change from "Entity" to "Country" is done earlier in the process, because the visualisations are stored in variables / vectors (see "Task number 2 - New visualisation").

# Here i move the legend to the bottom of the visualisation.
lterm_trend_facet + theme(legend.position = "bottom")
...
## Task number 5 - Floating table of contents & timestamp  

To create the floating table of contents to my Rmarkdown document, i write the following in the YAML:  
...
"toc: true" = creates the table of contents   
"toc_float: true" = makes the table of contents float   
The theme makes it red.  

To make the timestamp show when the document was updated last, i write this in the "date:" section of the YAML:  

"created 4 October 2021, updated r Sys.Date()"  

The "r Sys.Date()", gets the time the Rmarkdown file was knitted last, and displays it.
